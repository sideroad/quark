 ...
 *
 */
(function(b,a){qui=a.ui={list:[],attach:function(c,g){var j=qui.list,h=j.length,f,e,d;for(f=0;f<h;f++){d=qui.list[f];e=g[d];if(e){qui[d](c,e)}}},multi:function(c){var d=new Date().getTime()+""+parseInt(Math.random()*10000,10),e=c[0].id;c.find("[id]").each(function(){this.id=this.id+d});c.find("[data-quark-event]").each(function(){var m=b(this),g=m.data("quarkEvent").replace(/\s/,"").split(","),l=g.length,k,j=/^[^-]+-[^\.]+\.[^\:]+\:.+$/,h=[],n="",f;for(f=0;f<l;f++){k=g[f];if(j.test(k)){k=k+d}h.push(k)}n=h.join(", ");m.data("quarkEvent",n).attr("data-quark-event",n)});c.addClass(e);c.attr("id",e+d)}}})(jQuery,quark);
/*!
 ...
 *   jquery.ui.dialog.js
 */
(function(e,d){var a=d.core,c=d.ui,b={};c.list.push("dialog");c.dialog=function(f,h){var g={},l=f.attr("id"),j,k=false,i;h=e.extend(true,{singleton:true,focus:false,modal:false,close:false},h);i=h.singleton;if(b[l]&&h.singleton){f.dialog("moveToTop");return}if(!h.singleton){c.multi(f)}j=h.close;if(j){a.setIntercept(j,(i)?function(){k=true;f.dialog("close").remove();b[l]=false}:function(n,o){var m=f.attr("id");if(o.closest("[id='"+m+"']").length){k=true;f.dialog("close").remove();b[l]=false;return}return false})}g={modal:(h.modal)?true:false,draggable:(i)?false:true,resizable:(i)?false:true,width:(h.width)?h.width:"auto",height:(h.height)?h.height:"auto",beforeClose:function(){if(j&&!k){a.call(j,{},f);return false}}};f.dialog(g);b[l]=true}})(jQuery,quark);
/*!
