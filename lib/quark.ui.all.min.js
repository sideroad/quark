 ...
 * 
 */
(function(d){var c={"top-left":10,"top-right":10,"bottom-left":10,"bottom-right":10},e=10,b={"top-left":[],"top-right":[],"bottom-left":[],"bottom-right":[]},a=function(k){var n,g=k.attr("id"),v=k.data("floating-message"),h={},j=0,o=v.range,p=v.position,l=v.duration,u=v.stuffEasing,q=v.hide,s=v.close,m=v.timerId,f=v.height+v.margin+(v.padding*2),r=b[v.position];if(m){clearTimeout(m)}for(n=0;n<r.length;n++){v=r[n];if(g==v.id){j=n;continue}if(v.range>o){v.range-=f;h[v.verticalAlign]=v.range;v.elem.stop().animate(h,{duration:l,easing:u,queue:false})}}r.splice(j,1);c[p]-=f;k.stop().hide(q,l,function(){d(this).remove();if(s){s()}})};d.floatingMessage=function(j,g){var l="jqueryFloatingMessage"+new Date().getTime()+""+parseInt(Math.random()*10000,10),i=d('<div id="'+l+'" class="ui-widget-content ui-corner-all ui-floating-message"></div>'),h={},f=0,k=0;g=g||{};g=d.extend(true,{position:"top-left",verticalAlign:(g.position||"top-left").split("-")[0],align:(g.position||"top-left").split("-")[1],width:300,height:50,time:false,show:"drop",hide:"drop",padding:10,margin:10,duration:500,stuffEasing:"easeOutBounce",body:d("<div></div>"),close:false,click:a,elem:i,timerId:false,id:l},g);g.range=c[g.position];f=g.height;k=g.padding;if(j){g.body.html(j.replace(/\n/g,"<br />"))}if(g.className){i.addClass(g.className)}h={width:g.width+"px",height:f+"px",position:"fixed",padding:k+"px"};h[g.verticalAlign]=c[g.position];h[g.align]=e;i.css(h).append(g.body);i.bind("destroy.fms",function(){a(i)});d(document.body).append(i);if(g.click){i.bind("click.fms",function(){g.click(i)})}i.show(g.show,g.duration,function(){if(g.time){g.timerId=setTimeout(function(){g.click(i)},g.time)}});b[g.position].push(g);c[g.position]+=(f+g.margin+(k*2));i.data("floating-message",g);return g.body};d.fn.floatingMessage=function(f){return this.each(function(){if(typeof f=="string"){d(this).parent(".ui-floating-message").trigger(f+".fms")}else{f=f||{};f.body=this;d.floatingMessage(false,f)}})}})(jQuery);
/*!
 ...
 *
 */
(function(c,b){var a={};b.ui={list:[],attach:function(d,h){var k=a.list,j=k.length,g,f,e;for(g=0;g<j;g++){e=a.list[g];f=h[e];if(f){a[e](d,f)}}},multi:function(d){var e=new Date().getTime()+""+parseInt(Math.random()*10000,10),f=d[0].id;d.find("[id]").each(function(){this.id=this.id+e});d.find("[data-quark-event]").each(function(){var n=c(this),h=n.data("quarkEvent").replace(/\s/,"").split(","),m=h.length,l,k=/^[^-]+-[^\.]+\.[^\:]+\:.+$/,j=[],o="",g;for(g=0;g<m;g++){l=h[g];if(k.test(l)){l=l+e}j.push(l)}o=j.join(", ");n.data("quarkEvent",o).attr("data-quark-event",o)});d.addClass(f);d.attr("id",f+e)}};a=b.ui})(jQuery,quark);
/*!
 ...
 *   jquery.ui.button.js
 */
(function(c,b){var a=b.ui;a.list.push("button");a.button=function(d){d.find("input[type=button]").button()}})(jQuery,quark);
/*!
 ...
 *   jquery.ui.dialog.js
 */
(function(f,e){var a=e.core,b=e.controller,d=e.ui,c={};d.list.push("dialog");d.dialog=function(g,i){var h={},m=g.attr("id"),k,l=false,j;i=f.extend(true,{singleton:true,focus:false,modal:false,close:false},i);j=i.singleton;if(c[m]&&i.singleton){g.dialog("moveToTop");return}if(!i.singleton){d.multi(g)}k=i.close;if(k){a.setIntercept(k,(j)?function(){l=true;g.dialog("close").remove();c[m]=false}:function(o,p){var n=g.attr("id");if(p.closest("[id='"+n+"']").length){l=true;g.dialog("close").remove();c[m]=false;return}return false})}h={modal:(i.modal)?true:false,draggable:(j)?false:true,resizable:(j)?false:true,width:(i.width)?i.width:"auto",height:(i.height)?i.height:"auto",beforeClose:function(){if(k&&!l){b.call(k,{},g);return false}}};g.dialog(h);c[m]=true}})(jQuery,quark);
/*!
 ...
 */
(function(c,b){var a=b.ui;a.list.push("floatingMessage");a.floatingMessage=function(d,e){a.multi(d);d.floatingMessage(e)}})(jQuery,quark);