 ...
 *
 */
(function(b,a){qut=a.util={list:[],attach:function(c,g){var j=qut.list,h=j.length,f,e,d;for(f=0;f<h;f++){d=qut.list[f];e=g[d];if(e){qut[d](c,e)}}}}})(jQuery,quark);
/*!
 ...
 *   jquery.upload.js
 */
(function(d,c){var b=c.util,a=this;b.list.push("file");b.file=function(f){var g=a.particle,e='[data-quark-event^="file"]',h=f.find(e)||[];if(f.is(e)){h.push(f)}h.each(function(){var k=d(this),n=k.data("quarkEvent").replace(/\s/,"").match(/^file-([^\.]+)\.([^\?]+)(\?.+|)$/),j=n[1],m=n[2],l=n[3],o=j+"."+m,i=g.url.replace("${quark}",j).replace("${action}",m);file=d('<input type="file" name="file" style="position:absolute;visibility:hidden;" >').change(function(){d(this).upload(i+l,function(p){console.log("file uploaded");qc.call(o,p)},"json")});k.after(file);k.click(function(){file.click()})})}})(jQuery,quark);
/*!
